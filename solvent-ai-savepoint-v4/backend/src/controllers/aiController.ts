...
const chatSchema = z.object({
  provider: z.enum(['gemini', 'ollama', 'deepseek']),
  model: z.string(),
...
export class AIController {
  ...
  static async chat(req: Request, res: Response) {
    try {
      const { provider, model, messages, image, mode, smartRouter, fallbackModel, temperature, maxTokens } = chatSchema.parse(req.body);
      ...
      // 4. Global Graphing Instruction (Subtle)
      const globalGraphingPrompt = `
      [SYSTEM: KNOWLEDGE MAPPING]
      If the user's query involves complex relationships or multiple entities, occasionally append a <graph_data> block at the end of your response.
      Format: <graph_data>{ "nodes": [{"id": "unique_id", "title": "Label"}], "edges": [{"source": "id1", "target": "id2"}] }</graph_data>
      Only include the most important connections.
      `;
      lastMessage = `${globalGraphingPrompt}\n\n${lastMessage}`;
      ...
      if (provider === 'gemini') {
        try {
            if (image || mode === 'vision') {
              ...
              const response = await geminiService.generateVisionContent(lastMessage, [imagePart], model, temperature, maxTokens);
            ...
            const response = await geminiService.generateChatCompletion(messagesWithContext, model, shouldSearch, temperature, maxTokens);
            res.json({ response, model });
             ...
             if (isQuotaError) {
                 ...
                 const response = await ollamaService.generateChatCompletion(targetLocalModel, messages, temperature, maxTokens);
        ...
        const messagesWithContext = [...messages];
        messagesWithContext[messagesWithContext.length - 1].content = lastMessage;
        const response = await ollamaService.generateChatCompletion(model, messagesWithContext, temperature, maxTokens);
        res.json({ response: response.message.content });

      } else if (provider === 'deepseek') {
        const messagesWithContext = [...messages];
        messagesWithContext[messagesWithContext.length - 1].content = lastMessage;
        const response = await deepseekService.generateChatCompletion(messagesWithContext, model, temperature, maxTokens);
        res.json({ response });
      }
