...
export class AIService {
  ...
  async processChat(data: any) {
    const { provider, model, messages, image, mode, smartRouter, fallbackModel, temperature, maxTokens, deviceInfo } = data;
...
`;
  ...
  private async handleGeminiChat(messages: any[], model: string, image: any, mode: any, shouldSearch: boolean, fallbackModel: any, temp: any, maxTokens: any) {
    const GEMINI_CHAIN = ['gemini-3-pro-preview', 'gemini-3-flash-preview', 'gemini-1.5-pro'];
    ...
    try {
      ...
      console.error("Gemini failed, trying local fallback:", e.message);
  ...
  async generateImage(prompt: string, model?: string, host?: string, protocol?: string) {
    ...
    return { imageUrl: `${protocol}://${host}/generated_images/${fileName}`, fileName };
  }
