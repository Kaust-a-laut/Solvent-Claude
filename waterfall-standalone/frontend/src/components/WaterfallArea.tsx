...
import { Sparkles, X, RotateCcw, FlaskConical, ArrowLeftRight } from 'lucide-react';
...
import type { WaterfallModelChoice } from '../store/waterfallSlice';

// ─── Waterfall model config (mirrored from backend WATERFALL_CONFIG) ─────────
// Keep in sync with backend/src/constants/models.ts WATERFALL_CONFIG
...
const WATERFALL_MODEL_INFO: Record<StageKey, { displayName: string; A: ModelOptionInfo; B: ModelOptionInfo }> = {
  architect: {
    displayName: 'Planner',
    A: { label: 'Gemini 2.5 Pro',   score: 'GPQA 84%' },
    B: { label: 'GPT-OSS 120B',     score: 'MMLU-Pro 90%' },
  },
  reasoner: {
    displayName: 'Strategist',
    A: { label: 'Gemini 2.5 Flash', score: '232 t/s' },
    B: { label: 'Qwen3 32B',        score: '535 t/s' },
  },
  executor: {
    displayName: 'Executioner',
    A: { label: 'Claude Sonnet 4',   score: 'SWE-bench 73%' },
    B: { label: 'Qwen3 Coder+',     score: 'SWE-bench 71%' },
  },
  reviewer: {
    displayName: 'Reviewer',
    A: { label: 'Gemini 3.1 Pro',   score: 'GPQA 94%' },
    B: { label: 'Claude Sonnet 4',   score: 'SWE-bench 73%' },
  },
};
    ...
    <div className="flex flex-col h-full bg-black/20 backdrop-blur-3xl overflow-y-auto scrollbar-thin">
      ...
      <div className={cn(
        ...
        <div className="max-w-3xl w-full mx-auto flex flex-col gap-6">
          ...
          {/* ── Model Selector Panel (visible when pipeline is idle) ─────── */}
          <AnimatePresence>
            {!isActive && !isStreaming && (
              <motion.div
                key="model-selector"
                ...
                <div className="p-5 space-y-4">
                  ...
                  <div className="grid gap-2.5">
                    {STAGE_ORDER.map((stage) => {
                      const info = WATERFALL_MODEL_INFO[stage];
                      const stageConfig = STAGE_CONFIGS[stage];
                      const choice = waterfallModelSelection[stage];
                      ...
                      return (
                        <div key={stage} className="flex items-center gap-3">
                          ...
                          <div className="flex items-center gap-2 w-28 shrink-0">
                            <div className={cn('w-6 h-6 rounded-lg flex items-center justify-center', stageConfig.bgColor)}>
                              <Icon size={12} className={stageConfig.textColor} />
                            </div>
                          ...
                          <div className="flex-1 flex gap-1.5">
                            {(['A', 'B'] as WaterfallModelChoice[]).map((opt) => {
                              const isSelected = choice === opt;
                              const optInfo = info[opt];
                              return (
                                <button
                                  key={opt}
                                  onClick={() => setWaterfallModelChoice(stage, opt)}
                                  className={cn(
                                    'flex-1 flex items-center justify-between px-3 py-2 rounded-xl border transition-all duration-200',
                                    isSelected
                                      ? `bg-${stageConfig.color}/10 border-${stageConfig.color}/30`
                                      : 'bg-white/[0.02] border-white/[0.06] hover:border-white/15 hover:bg-white/[0.04]',
                                  )}
                                  style={isSelected ? {
                                    borderColor: `${stageConfig.borderRgba}0.35)`,
                                    background: `${stageConfig.borderRgba}0.06)`,
                                  } : undefined}
                                >
                                  <div className="flex items-center gap-2">
                                    <div className={cn(
                                      ...
                                      isSelected
                                        ? `border-${stageConfig.color}`
                                        : 'border-white/20',
                                    )} style={isSelected ? { borderColor: `${stageConfig.borderRgba}0.7)` } : undefined}>
                                      {isSelected && (
                                        <div
                                          className="w-2 h-2 rounded-full"
                                          style={{ background: `${stageConfig.borderRgba}0.9)` }}
                                        />
                                      )}
                                    </div>
                                    <span className={cn(
                  ...
                  <div className="pt-2 border-t border-white/[0.04]">
                    <span className="text-[8px] text-slate-700 font-mono">
                      Selected model runs primary · other serves as automatic fallback
                    </span>
                  </div>
                </div>
              </motion.div>
