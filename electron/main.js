...
// electron/ModelManager.ts
var import_electron2 = require("electron");
var store = new ElectronStore();
var calculateCost = (model, tokens) => {
  if (model.includes("free") || model.includes("ollama")) return 0;
  if (model.includes("gpt-4") || model.includes("opus")) return tokens / 1e3 * 0.03;
  return tokens / 1e3 * 1e-3;
};
var API = {
  call: async (modelStr, messages) => {
    let provider = "gemini";
...
var ModelManager = {
  ...
  execute: async (tier, messages) => {
    ...
    try {
      ...
      import_electron2.BrowserWindow.getAllWindows().forEach((w) => w.webContents.send("model-status", { status: "primary", model: pref.primary }));
      return result;
    } catch (err) {
      if (pref.autoShift && ModelManager.isRecoverable(err)) {
        ...
        import_electron2.BrowserWindow.getAllWindows().forEach((w) => w.webContents.send("model-status", { status: "fallback", model: pref.fallback }));
  ...
  trackUsage: (model, tokens) => {
    const current = store.get("usage", { tokens: 0, cost: 0, requests: 0 });
...
async function createWindow() {
  ...
  const NOTES_FILE = import_path.default.join(__dirname, "../../.solvent_notes.md");
  let notepadBuffer = "";
  let active_mission_context = {
    mission_id: Date.now(),
  ...
  const globalHistory = [];
  import_electron3.ipcMain.on("report-activity", (event, activity) => {
  ...
  import_electron3.ipcMain.on("log-action", (event, action) => {
    import_electron3.ipcMain.emit("report-activity", event, action);
  });
  import_electron3.ipcMain.handle("get-mission-context", () => active_mission_context);
  import_electron3.ipcMain.on("set-mission-goal", (event, goal) => {
  ...
  import_electron3.ipcMain.handle("model:get-preference", (_, tier) => ModelManager.getPreference(tier));
  import_electron3.ipcMain.handle("model:set-preference", (_, tier, prefs) => ModelManager.setPreference(tier, prefs));
  import_electron3.ipcMain.handle("model:get-usage", () => ModelManager.getUsage());
  import_electron3.ipcMain.handle("model:reset-usage", () => ModelManager.resetUsage());
  try {
    if (import_fs.default.existsSync(NOTES_FILE)) {
      notepadBuffer = import_fs.default.readFileSync(NOTES_FILE, "utf-8");
    } else {
      import_fs.default.writeFileSync(NOTES_FILE, "");
    }
  } catch (e) {
  ...
  import_fs.default.watch(NOTES_FILE, (eventType, filename) => {
    if (filename && eventType === "change") {
      ...
      try {
        const newContent = import_fs.default.readFileSync(NOTES_FILE, "utf-8");
  ...
  const loadWithRetry = (window, url) => {
